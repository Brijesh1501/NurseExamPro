...
export const QF={sec:null,edit:null};   // question-bank selection state
...
export async function secOptions(){const stg=await ok(db.from('exam_stages').select('id,name,exams(name),sections(id,name,ord)').order('name'));
 if(!QF.sec)QF.sec=stg.flatMap(s=>s.sections)[0]?.id;
 return{stg,html:stg.map(s=>`<optgroup label="${esc(s.exams.name+' — '+s.name)}">${s.sections.sort((a,b)=>a.ord-b.ord).map(x=>`<option value=${x.id} ${x.id===QF.sec?'selected':''}>${esc(x.name)}`).join('')}</optgroup>`).join('')}}
