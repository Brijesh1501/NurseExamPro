...
export async function aQuestions(){adm('<p>Loading…</p>');const{html}=await secOptions();
 const rows=QF.sec?await ok(db.from('questions').select('*').eq('section_id',QF.sec).order('created_at',{ascending:false}).limit(200)):[],e=QF.edit||{options:['','','',''],correct:0,active:true};
 $('.wrap').insertAdjacentHTML('beforeend',`<label>Section<select id=sc>${html}</select></label>
 ...
 $('#da').onclick=async()=>{if(confirm('Delete ALL questions in this section?')){await ok(db.from('questions').delete().eq('section_id',QF.sec));aQuestions()}};
 ...
 $('#qf').onsubmit=async ev=>{ev.preventDefault();const f=ev.target,fd=new FormData(f);let url=fd.get('url')||null;const file=fd.get('file');
  ...
  const o={section_id:QF.sec,question:fd.get('question'),options:[0,1,2,3].map(i=>fd.get('o'+i)),correct:+fd.get('correct'),explanation:fd.get('explanation')||null,image_url:url,needs_image:QF.edit?.needs_image&&!url,active:!!fd.get('active')};
