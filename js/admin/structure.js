...
export async function aStruct(id,sid){adm('<div id=gs></div><div id=g></div><div id=g2></div>');
 const[e,st,ts]=await Promise.all([ok(db.from('exams').select('name').eq('id',id).single()),ok(db.from('exam_stages').select('*').eq('exam_id',id).order('ord')),ok(db.from('test_series').select('*').eq('exam_id',id).order('ord'))]);
 $('#gs').innerHTML=`<h3>${esc(e.name)} · Test series</h3><p class=muted>Each series is a separate paper (Mock Test 1, 2, 3…) with its own questions. Tick Shuffle to randomise question order per attempt.</p><div id=gsg></div>`;
 grid($('#gsg'),'test_series',ts,[['name','Series name'],['ord','Order','number'],['description','Description'],['shuffle','Shuffle','bool'],['is_active','Active','bool']],{exam_id:id,ord:ts.length+1,shuffle:false,is_active:true},x=>`<a class="btn ghost" href="#/admin/questions" data-ser="${x.id}">Questions</a>`,()=>aStruct(id,sid));
 $('#gsg').addEventListener('click',ev=>{const a=ev.target.closest('[data-ser]');if(a)QF.ser=a.dataset.ser});
