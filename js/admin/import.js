...
export async function aImport(){adm('<p>Loading…</p>');const{stg}=await secOptions();
 ...
 <label>Target stage<select id=ts>${stg.map(s=>`<option value=${s.id}>${esc(s.exams.name+' — '+s.name)}`).join('')}</select></label><input type=file id=fi accept=.csv> <a href=# id=tpl>Download template</a><div id=pv></div></div>`);
 ...
 $('#fi').onchange=e=>Papa.parse(e.target.files[0],{header:true,skipEmptyLines:true,complete:({data})=>{
  ...
  data.forEach((r,i)=>{const sn=String(r.Section||'').trim(),m=sn.match(/(\d+)\s*$/),sec=S[m?m[1]-1:-1]||S.find(x=>x.name.toLowerCase()===sn.toLowerCase());
   ...
   rows.push({section_id:sec.id,question:r.Question.trim(),options:op,correct:ci,explanation:r.Explanation||null,image_url:/^(https?:|data:)/i.test(img)?img:null,needs_image:img===''})});
  ...
  $('#go').onclick=async()=>{$('#go').disabled=true;const ex=new Set((await ok(db.from('questions').select('section_id,question').in('section_id',S.map(x=>x.id)))).map(x=>x.section_id+x.question));
   ...
   alert(`Imported ${fresh.length}, skipped ${rows.length-fresh.length} duplicates.`);location.hash='#/admin/questions'}}})}
