...
function parseRow(r,i,Sections,err){
  ...
  // Normalize "N/A", "NA", "none", or "-" entries to an empty string so they aren't flagged as missing image files
  const cleanImg = /^(n\/?a|none|-)$/i.test(img) ? '' : img;
  ...
  return{
    ...
    _img:cleanImg,
    _imgFile:null,
    image_url:/^(https?:|data:)/i.test(cleanImg)?cleanImg:null,
    needs_image:false
  };
}
...
export async function aImport(){adm('<p>Loading…</p>');const{stg,series,serHtml}=await secOptions();
  ...
  function preview(rows,err,imgFiles){
   ...
   rows.forEach(r=>{
     if(!r) return;
     if(imgFiles && r._img && !r.image_url){
       const f=imgFiles.get(r._img.split(/[\\/]/).pop().toLowerCase());
       if(f){r._imgFile=f;r.needs_image=false}
       else{r.needs_image=true}
     }
   });
   rows=rows.filter(Boolean);
   const missing=rows.filter(r=>r.needs_image).length,toUpload=rows.filter(r=>r._imgFile).length;
   $('#pv').innerHTML=`<p><b class=ok>${rows.length} valid</b>, <b class=bad>${err.length} errors</b>${toUpload?`, <b>${toUpload} images to upload</b>`:''}${missing?`, <b class=bad>${missing} image not found</b>`:''}</p>
