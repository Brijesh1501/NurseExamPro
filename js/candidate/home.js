...
export async function vHome(){shell('<p>Loading…</p>');
 ...
 const ip=await ok(db.from('attempts').select('id,test_series(name),exam_stages(name,exams(name))').eq('status','in_progress'));
 $('.wrap').innerHTML=`${ip.length?`<div class=card><b>Tests in progress</b>${ip.map(a=>`<p>${esc(a.exam_stages.exams.name)} · ${esc(a.test_series?.name)} · ${esc(a.exam_stages.name)} <a class=btn href="#/test/${a.id}">Resume</a></p>`).join('')}</div>`:''}<h2>Choose an examination</h2><div class=grid>${ex.map(e=>`<a class=card href="#/exam/${e.id}" style="text-decoration:none;color:inherit"><h3 style="margin:0">${esc(e.name)}</h3><span class=muted>${esc(e.org)}</span></a>`).join('')}</div>`}

export async function vExam([id]){shell('<p>Loading…</p>');
 const e=await ok(db.from('exams').select('*,test_series(*),exam_stages(*,sections(*))').eq('id',id).single());
...
export async function vHistory(){shell('<p>Loading…</p>');
 const a=await ok(db.from('attempts').select('id,status,score,total,submitted_at,started_at,test_series(name),exam_stages(name,exams(name))').eq('user_id',S.me.id).order('started_at',{ascending:false}));
